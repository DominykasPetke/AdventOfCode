...
int main()
{
    ...
    std::unordered_map<int, std::unordered_map<int, std::unordered_map<int, bool>>> space;
    ...
    for (int i = 0; i < 6; i++)
    {
        std::unordered_map<int, std::unordered_map<int, std::unordered_map<int, bool>>> new_space;
            ...
            for (int y = sspace_y_min; y <= sspace_y_max; y++)
            {
                for (int z = sspace_z_min; z <= sspace_z_max; z++)
                {
                    int neighbours = [x, y, z, &space]() {
                        ...
                        return sum - space[x][y][z];
                    }();

                    if (space[x][y][z])
                    {
                        new_space[x][y][z] = (neighbours == 2 || neighbours == 3);
                    }
                    else
                    {
                        new_space[x][y][z] = (neighbours == 3);
                    }
