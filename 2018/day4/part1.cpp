...
int main()
{
	...
	std::vector<std::pair<int, int>> sleepSum;
	...
	for (std::map<int, sleepTimes>::iterator it = sleeps.begin(); it != sleeps.end(); ++it)
	{
		...
		for (int i = 0; i < 60; i++)
			sum += it->second.sleepTime[i];

		sleepSum.push_back({it->first, sum});
	}

	std::sort(sleepSum.begin(), sleepSum.end(), [](std::pair<int, int> const &t, std::pair<int, int> const &f) { return t.second > f.second; });

	int max = -1;
	for (int i = 0; i < 60; i++)
		if (sleeps[sleepSum[0].first].sleepTime[i] > max)
		{
			max = sleeps[sleepSum[0].first].sleepTime[i];
			id = i;
		}

	std::cout << id * sleepSum[0].first << '\n';
	return 0;
}