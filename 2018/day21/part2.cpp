...
int main()
{
    ...
    std::vector<long long int> possible_ans;
    ...
    while (registers[instruction_pointer] < (int)code.size())
    {
        ...
        {
            if (std::find(possible_ans.begin(), possible_ans.end(), registers[1]) == possible_ans.end())
                possible_ans.push_back(registers[1]);
            else
            {
                std::cout << possible_ans.back() << '\n';
                return 0;
            }
        }
    }
