...
struct Unit
{
    ...
    Unit(std::size_t new_X, std::size_t new_Y, int strength, bool goblin)
        : x(new_X), y(new_Y), attack_strength(strength), health_points(200), is_goblin(goblin) {}
...
bool simulate(int att)
{
    ...
    while (std::getline(in, line)) // input map and starting unit positions
    {
        ...
        {
            units.push_back({search, map.size(), att, false});
            line[search] = '.';
            search = line.find('E');
        }
        ...
        while (search != std::string::npos)
        {
            units.push_back({search, map.size(), 3, true});
            line[search] = '.';
            search = line.find('G');
        }
    ...
    int elf_count = std::count_if(units.begin(), units.end(), [](Unit const &a) { return !a.is_goblin; });
        ...
        for (auto &unit : units) // make each unit's turn
            if (unit.health_points > 0)
            {
                ...
                if (targets.size() == 0) // if no targets end the game
                {
                    ...
                    if (elf_count != std::count_if(units.begin(), units.end(), [](Unit const &a) { return !a.is_goblin; }))
                        return true;
